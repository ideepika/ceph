...
void TokenEngine::AcceptedServiceTypesCache::refresh(CephContext* const cct)
{
  services.clear();
  ...
  for (auto& s : parsed) {
    ...
    if (!s.empty()) {
      services.insert(std::move(s));
    }
  }
}

bool
TokenEngine::check_access_rules(
    const DoutPrefixProvider* dpp,
    const req_state* s,
    const token_envelope_t& token) const
{
  if (token.get_access_rules().empty()) {
    return true;
  }
  ...
  // If access rules disabled, allow
  if (!dpp->get_cct()->_conf->get_val<bool>("rgw_keystone_verify_access_rules")) {
    return true;
  }

  for (const auto& rule : token.get_access_rules()) {
    // Parse accepted service types
    if (!accepted_service_types.contains(rule.service)) {
      continue;
    }

    if (rule.method == s->info.method && path_matches_pattern(rule.path, path)) {
  ...
  ldpp_dout(dpp, 5) << "No access rule matched for method=" << s->info.method
                    << " path=" << path << dendl;
  return false;
}
